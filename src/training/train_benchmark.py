...
RESULTS_FILE = 'results_v2_chips_reproduced.csv'
PREDS = {}
...
def main():
    ...
    log("=== GAT: no-GDELT + (keepall/topk) x (concat/blend/attention) -- matches run_shrinkage_head.py's GAT_TARGETS ===")
    run_once('GAT_first_none', run_graph,
             'GAT', train_data, data_2023, False, 'none', None, agg_mode='first', device=torch.device('cpu'))
    for scoring, gfile in [('keepall', KEEPALL), ('topk', TOPK)]:
        for fusion in ['concat', 'blend', 'attention']:
            run_once(f'GAT_first_{scoring}_{fusion}', run_graph,
                     'GAT', train_data, data_2023, True, scoring, gfile, fusion=fusion, agg_mode='first', device=torch.device('cpu'))

    log("=== EdgeGAT_full: 4 combos x (concat/blend/attention) -- matches run_shrinkage_head.py's EDGE_TARGETS ===")
    for combo, node_file, pair_file in SCORER_COMBOS:
        for fusion in ['concat', 'blend', 'attention']:
            run_once(f'EdgeGAT_full_{fusion}_{combo}', run_edge_full,
                     'EdgeGAT', train_data, data_2023, combo, node_file, pair_file,
                     device=torch.device('cpu'), node_fusion=fusion)
